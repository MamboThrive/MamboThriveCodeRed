...
# Utility: Check if PDF is searchable (has extractable text)
def is_pdf_searchable(pdf):
    try:
        # Read PDF bytes
        if isinstance(pdf, str):
            if not os.path.isfile(pdf):
                print(f'is_pdf_searchable error: file path does not exist: {pdf}')
                return False
            ...
            print(f'is_pdf_searchable error: unsupported input type {type(pdf)}')
            return False
        ...
        try:
            ...
            if not reader.pages:
                print('is_pdf_searchable error: PDF has no pages')
            else:
                for i, page in enumerate(reader.pages):
                    try:
                        text = page.extract_text()
                        if text and text.strip():
                            print(f'is_pdf_searchable: found text on page {i} using PyPDF2')
                            return True
                    except Exception as e:
                        print(f'is_pdf_searchable warning: page {i} failed with {e}')
        except Exception as e:
            print(f'is_pdf_searchable warning: PyPDF2 failed with {e}')
        ...
        try:
            ...
            for i, page in enumerate(doc):
                text = page.get_text()
                if text and text.strip():
                    print(f'is_pdf_searchable: found text on page {i} using PyMuPDF')
                    return True
        except Exception as e:
            print(f'is_pdf_searchable fallback error (PyMuPDF): {e}')

        print('is_pdf_searchable: no extractable text found in PDF')
        return False
